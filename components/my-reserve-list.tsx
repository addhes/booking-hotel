    ...
    <div>
      {reservation.map((item) => (
        <div className="bg-white shadow pb-4 mb-4 md:pb-0 relative" key={item.id}>
          <div className="flex item-center justify-between bg-gray-100 px-2 py-1 rounded-t-sm">
            <h1 className="text-sm font-medium text-gray-900 truncate">
              Reservaton ID #{item.id}
            </h1>
          ...
          <div className="flex flex-col mb-4 items-center bg-white rounded-sm md:flex-row md:w-full">
            <Image
              ...
              width={500}
              height={300}
              className="object-cover w-full rounded-t-sm h-60 md:h-auto md:w-1/3 md:rounded-none
            md:rounded-s-sm"
              alt="Imageroom"
            />
          ...
          <div className="flex items-end justify-end absolute inset-4">
            {item.Payment?.status === "unpaid" ? (
              <Link href={`/checkout/${item.id}`} className="px-4 py-1 bg-orange-400 text-white rounded-sm hover:bg-orange-500">Pay Now</Link>
            ) : (
              <Link href={`/myreservation/${item.id}`} className="px-5 py-1 bg-orange-400 text-white rounded-sm hover:bg-orange-500">View Detail</Link>
            )}
          </div>
        </div>
      ))}
